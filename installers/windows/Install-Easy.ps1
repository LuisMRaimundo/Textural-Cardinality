...
Write-Host ' Textural_Cardinality — Installer' -ForegroundColor Cyan
...
"@
    ...
    @"
Textural_Cardinality standard install
...
"@ | Set-Content -LiteralPath (Join-Path $InstallRoot 'install-info.txt') -Encoding UTF8
    ...
    Write-Host ' Start: Desktop or Start menu → Textural_Cardinality'
    Write-Host ''

    if (-not $NoLaunch) {
        $launch = Join-Path $InstallRoot 'Launch-Textural_Cardinality.bat'
        if (Test-Path $launch) {
            Write-InstallLog 'Launching Textural_Cardinality…'
